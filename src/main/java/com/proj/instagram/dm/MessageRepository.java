...
public interface MessageRepository extends JpaRepository<Message, Long> {
	 ...
	 * ) List<Message> getMessages(@Param("username1") String
	 * username1, @Param("username2") String username2);
	 */
	@Query(value = "SELECT m.id, m.sender_username , m.receiver_username, m.content, m.timestamp " + "FROM ( "
			+ "    SELECT m.*, " + "           ROW_NUMBER() OVER (PARTITION BY CASE "
			+ "               WHEN sender_username  = :userSender THEN receiver_username  "
			+ "               ELSE sender_username  " + "           END ORDER BY timestamp DESC) as rn "
			+ "    FROM message m " + "    WHERE sender_username  = :userSender OR receiver_username  = :userSender "
			+ ") m " + "WHERE rn = 1", nativeQuery = true)
